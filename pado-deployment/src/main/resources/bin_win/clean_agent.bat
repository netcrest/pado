...
if "%1" == "-?" (
   ...
   echo.
   echo    clean_agent [-grid %GRIDS_OPT%] [-site %SITES_OPT%] [-?]
   echo.
   echo        Cleans up agent directories by removing log, stats, other temporary files
   echo        from the specified grid and site.
   echo.
   echo    Default: clean_agent -grid %GRID_DEFAULT% -site %SITE_DEFAULT%
   echo.
   goto stop
)
