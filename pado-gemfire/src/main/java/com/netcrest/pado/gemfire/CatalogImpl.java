...
public class CatalogImpl implements ICatalog
{
			...
			RegionService padoRegionService, Pool padoPool, Pool indexMatrixPool, Region routerRegion)
	{
		...
		this.gridService = new GemfireGridService(appId, credentials, token, loginInfo.getUsername(), padoRegionService, padoPool, indexMatrixPool, routerRegion);
			...
			for (BizInfo bizInfo : bizInfoSet) {
				if (bizInfo.getBizInterfaceName() != null) {
					try {
						...
						BizManager<IBiz> bizManager = BizManagerFactory.getBizManagerFactory().createBizManager(bizClass, true);
