...
public class GemfireCatalogImpl implements ICatalog
{
			...
			RegionService padoRegionService, Pool padoPool, Pool indexMatrixPool, Region routerRegion)
	{
		...
		this.gridService = (GemfireGridService)InternalFactory.getInternalFactory().createGridService(null, appId, credentials, token, loginInfo.getUsername(), false);
		...
		if (bizInfoSet != null) {
			for (BizInfo bizInfo : bizInfoSet) {
				if (bizInfo.getBizInterfaceName() != null) {
					try {
						Class bizClass = bizInfo.getClass().getClassLoader().loadClass(bizInfo.getBizInterfaceName());
...
//			} catch (Exception e) {
//				Logger.error("Error occurred while creating a IBizLocal object: " + localImplClassName + ". Assigning a proxy object instead.", e);
//				return (T)bizManager.newClientProxy();
			}
		}
	}
	...
	private <T> T newInstanceLocalProxy(GemfireBizManager<IBiz> bizManager, IBizLocal bizLocal, Object...bizLocalArguments)
	{
	...
	private GemfireBizManager<IBiz> getBizManager(String bizInterfaceName)
	{
		...
		if (bizManager == null) {
			// TODO: isFuture() assumes all Future class names end with "Future".
			if (GemfireBizUtil.isFuture(bizInterfaceName)) {
				// Load the future class using the same IBiz target class loader.
				...
				if (bizManager != null) {
					...
					try {
						...
						Logger.warning("Attempted to load undefined Future biz class, " + bizInterfaceName + ". Aborted.");
					}
				}
			}
		}
	...
	public Class<?>[] getAllBizClasses()
	{
		...
		int i = 0;
		for (GemfireBizManager gemfireRpcServiceMgr : col) {
			classes[i++] = gemfireRpcServiceMgr.getTargetClass();
		}
		return classes;
	}
