...
import com.netcrest.pado.info.PathAttributeInfo;
 ...
 *
 */
public class GemfireRegionAttributeInfo extends PathAttributeInfo implements DataSerializable
{
	...
	//diskstore
	public static final String DISK_STORE_NAME="disk_store_name";
	...
	public GemfireRegionAttributeInfo() {}
	
	public GemfireRegionAttributeInfo(Properties attrProperties)
	{
	...
	public RegionAttributes createRegionAttributes() 
	{
		...
		for (Iterator<Entry<String, String>> iterator = entrySet.iterator(); iterator.hasNext();) {
			...
			} else if (attr.equals(ENABLE_GATEWAY)) {
				factory.setEnableGateway(Boolean.parseBoolean(value));
			} else if (attr.equals(ENABLE_SUBSCRIPTION_CONFLATION)) {
				factory.setEnableSubscriptionConflation(Boolean.parseBoolean(value));
			} else if (attr.equals(HUB_ID)) {
				factory.setGatewayHubId(value);
			...
			} else if (attr.equals(DISK_STORE_NAME)) {
				factory.setDiskStoreName(value);
