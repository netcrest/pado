...
import com.netcrest.pado.gemfire.gateway.MultiGatewayHubRouterManager;

/**
 * CacheServerInitializer is a convenience utility class that 
 * initializes cache servers defined in the cache.xml file. It eliminates
 * the need of creating multiple cache.xml files that are differ only
 * by cache server settings. CacheServerInitializer achieves this by
 * allowing the startup script to pass in the cache server configuration
 * information via system properties.
 * <p>
 * The system properties must begin with the following default prefix excluding the quotes.
 * <pre>gfinit.cacheserver."</pre>
 * 
 * The prefix <code>"gfnit."</code> is reserved and cannot be changed. The default <code>"cacheserver."</code> 
 * prefix can be changed using the paremeter name <code>"system.property.prefix"</code> in the 
 * cache.xml file. For example,
 * <p>
 * <pre>
 *    &lt;region name="__init"&gt;
      &lt;region-attributes data-policy="empty"&gt;
         &lt;cache-listener&gt;
            &lt;class-name&gt; 
               com.netcrest.pado.gemfire.CacheInitializer 
            &lt;/class-name&gt;
            ...
            <font color="blue"><b>&lt;parameter name="cacheserver"&gt;
               &lt;declarable&gt;
                  &lt;class-name&gt;com.netcrest.pado.gemfire.CacheServerInitializer&lt;/class-name&gt;
 ...
 * </pre>
 * The above example defines two cache servers. Each cache server must be uniquely enumerated. 
 * Server groups must be in double quotes and comma separated. That means CacheServerInitializer
 * does not support server group names with commas.
 * There is no requirement for having the numbers in sequential order. The only requirement is
 * each cache server must have a unique number.
 * 
 * @author dpark
 *
 */
			...
			if (key.startsWith(prefix + ".")) {
				...
				if (split.length < 4) {
					...
					try {
						...
						logger.error("Invalid cache server initializer system property: " + key + ". A number must follow after the prefix.", ex);
					}
				}
			}
		}
...
//			  socket-buffer-size="">
		...
		for (Object object : keyNumSet) {
			...
			try {
				server.start();
				logger.config("CacheServer started via CacheServerInitializer: " + server);
	...
	private void initMultirouter()
	{
		String val = initProps.getProperty("enable-multirouter", "false");
		boolean enableMultirouter = val.equals("true") ? true : false;
		if (enableMultirouter) {
