...
public class ClientResults<T> implements Serializable, IScrollableResultSet<T>
{
	...
	private GridQueryServiceImpl serviceImpl;
	...
	public ClientResults(GridQuery query, GridQueryServiceImpl serviceImpl)
	{
		this.query = query;
		this.id = query.getId();
	...
	public void setGridQueryService(GridQueryServiceImpl serviceImpl)
	{
			...
			throws GridQueryResultSetExpiredException
	{
		...
		if (indexMatrix == null) {
			...
			if (serviceImpl.isIndexBuildingInProgress(query.getId())) {
	...
	public boolean goToSet(int index)
	{
		...
		try {
			...
			if (throwExceptionOnExpire) {
				...
				// Retry the query
				ClientResults newResultSet = (ClientResults) GridQueryService.getGridQueryService().query(query);
	...
	private List<T> toObjectList(boolean isDomainObject)
	{
		...
		if (obj instanceof TemporalEntry) {
			...
			if (isDomainObject) {
				domainObjectList = new ArrayList(list.size());
				for (Object object : list) {
					obj = createDomainObject(domainObjectClass, object);
					domainObjectList.add(obj);
				}
			}
		}
	...
	private void copyTo(ClientResults clientResults)
	{
