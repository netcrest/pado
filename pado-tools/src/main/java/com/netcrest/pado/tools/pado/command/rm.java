...
public class rm implements ICommand
{
	...
	public void help()
	{
		PadoShell.println("rm [-path <path>] <key-field-list> | [-?]");
		PadoShell.println("rm -buffer <name> <number-list>");
		PadoShell.println("   Remove entries from the current or specified path.");
		...
		PadoShell.println("     -buffer <name> <row-number-list>  Remove entries from the specified buffer using the");
	...
	{
		return "Remove one or more keys (entries) from current or specified path.";
	}
