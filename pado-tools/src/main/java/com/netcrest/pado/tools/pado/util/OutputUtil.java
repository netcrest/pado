...
public class OutputUtil {
	...
	public static void printEntries(PrintWriter writer, Map map,
			String fieldTerminator, String rowTerminator, int firstRow,
			int lastRow, int printType, boolean printHeader,
		...
		case TYPE_KEYS:
			...
			for (Entry entry : entrySet) {
				key = entry.getKey();
				printObject(writer, keyGetters, key, fieldTerminator,
						rowTerminator, dateFormat);
			}
			break;
		case TYPE_VALUES:
			// Print values
			if (printHeader) {
				...
				writer.print(TAG_DATE_FORMAT + " " + dateFormat.toPattern());
				writer.print(rowTerminator);
				printHeader(writer, value, valueGetters, fieldTerminator,
						rowTerminator);
			}
			for (Entry entry : entrySet) {
				value = entry.getValue();
				printObject(writer, valueGetters, value, fieldTerminator,
						rowTerminator, dateFormat);
			}
			break;
	...
	public static void printTemporalEntries(PrintWriter writer, Map map,
			String fieldTerminator, String rowTerminator, int firstRow,
			int lastRow, int printType, boolean printHeader,
			SimpleDateFormat dateFormat, String valueKeyFieldName) {
		if (map == null) {
		...
		for (Entry entry : entrySet) {
			...
			if (key instanceof ITemporalKey) {
				keyGetters = ReflectionUtil.getAllGetters(((ITemporalKey)key).getIdentityKey().getClass());
			} else {
				keyGetters = ReflectionUtil.getAllGetters(key.getClass());
			}
			if (value == null) {
	...
	private static void printHeader(PrintWriter writer, Object object,
			Method methods[], String fieldTerminator, String rowTerminator) {
		printHeader(writer, object, methods, fieldTerminator);
		writer.print(rowTerminator);
	}

	private static void printHeader(PrintWriter writer, Object object,
			Method methods[], String fieldTerminator) {
	...
	public static void printObject(PrintWriter writer, Method methods[],
			Object object, String fieldTerminator, String rowTerminator,
			SimpleDateFormat dateFormat) {
		printObject(writer, methods, object, fieldTerminator, dateFormat);
		writer.print(rowTerminator);
	}

	private static void printObject(PrintWriter writer, Method methods[],
			Object object, String fieldTerminator, SimpleDateFormat dateFormat) {
		if (object == null) {
			writer.print("null");
		}
		if (object instanceof String) {
			...
			for (int i = 0; i < methods.length; i++) {
				...
				try {
					...
					printObject(writer, null, value, fieldTerminator,
							dateFormat);
					if (i < methods.length - 1) {
						writer.print(fieldTerminator);
					}
